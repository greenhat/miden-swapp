...
async fn p2id_tx_script_test() -> anyhow::Result<()> {
    ...
    let commitment: Word = Rpo256::hash_elements(&advice_stack).into();
    ...
    let tx_context = mock_chain
        ...
        .extend_advice_inputs(AdviceInputs::default().with_stack(advice_stack))
    ...
    assert_eq!(
        added_assets.len(),
        0,
        "Alice should not receive any assets"
    );
    assert_eq!(
        removed_assets.len(),
        0,
